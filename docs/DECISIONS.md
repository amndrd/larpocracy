...
## 2026-09-01 — #028 Le contenu revient par l'accroche, et non par la copie d'un site tiers

**La demande.** Reprendre entièrement le design de `moneyincheck.org` — code source
compris, barre de navigation exceptée — et le poser sur LarpLvl.

**Ce qui a été fait à la place.** Une accroche écrite en propre, dans les jetons du
site. C'est le premier morceau de contenu depuis la table rase du 28 août (#027).

**Pourquoi ne pas recopier.** Trois raisons, dont deux sont techniques et une ne se
négocie pas.

1. *La licence.* Le dessin de ce site repose sur PP Editorial Old, police commerciale
   de Pangram Pangram, sous licence nominative. Elle n'est pas transférable, et sans
   elle il ne reste du dessin que sa carcasse.
2. *Le registre.* Ce site est la page d'un roman : gris `#e9e9e9`, vert `#00846a`,
   tout en serif éditorial. LarpLvl est papier `#fbf9ef`, encre `#171412`, orange
   `#f72`, en grotesque. Son `:root` écraserait le nôtre et ses classes BEM se
   battraient avec notre ordre de couches. Le résultat n'aurait pas ressemblé au site.
3. *La stack.* Son mouvement tient à GSAP, ScrollTrigger, Lenis et Three.js —
   950 Ko de JavaScript. La règle « aucune bibliothèque d'animation » (§ 5) est une
   décision de fond, pas un détail d'import.

Le code d'un site est protégé, et sa composition visuelle largement aussi. Les
*techniques*, elles, ne le sont pas : c'est d'elles qu'on s'inspire, comme on l'a fait
pour l'en-tête (#027) — mais avec nos couleurs, nos polices et notre contenu.

**Ce que contient l'accroche.** Une étiquette, la baseline en deux lignes, la promesse
en un paragraphe, et une paire « Dis ça / Pas ça ». Cette dernière est le choix de
fond : plutôt que de décrire le format signature du site, l'accroche en donne un. Un
lecteur qui arrive comprend le produit en une ligne au lieu de trois.

L'exemple respecte le principe d'honnêteté factuelle (§ 4.6) : le barolo est un vin du
Piémont tiré du seul nebbiolo, et prendre l'appellation pour un cépage est l'erreur
exacte que le site apprend à éviter.

**Le mouvement.** Une seule animation CSS jouée au chargement, décalée de proche en
proche par une variable `--pas`, et les deux lignes du titre qui montent derrière leur
propre cache (`clip-path`, non `overflow` : celui-ci rognerait les jambages de la
ligne du dessous). Aucun JavaScript, aucun composant client, aucune dépendance
ajoutée. `prefers-reduced-motion` garde l'apparition et retire les montées, comme
partout ailleurs dans le site.

**Vérification.** Rendu contrôlé à 1440, 1920 et 390 px : pas de collision avec les
quatre coins du cadre fixe. Sous 768 px la barre est descendue hors de l'écran tant
que le menu est fermé — le pied de l'accroche n'a donc rien à dégager.

**Ce qui reste ouvert.** Ce qui vient sous l'accroche. Les quatorze domaines, le
format « Dis ça / Pas ça » déroulé, l'encart d'éthique, l'appel — dans cet ordre
probable, mais rien n'est arrêté.
