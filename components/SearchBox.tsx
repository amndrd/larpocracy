...
const TONS: Record<SearchHit['kind'], string> = {
  ...
  terme: 'bg-white/[0.06] text-ink-2',
  ...
  return (
    <div ref={boxRef} className="relative w-full">
      ...
      <input
        ...
        spellCheck={false}
        autoFocus={autoFocus}
        className="h-10 w-full rounded-full bg-white/[0.06] pl-10 pr-4 ring-1 ring-white/10 ring-inset text-[0.875rem] text-ink transition-all duration-200 placeholder:text-ink-3 focus:bg-white/[0.10] focus:ring-white/25 focus:outline-none"
      />

      {open && active && (
        <div className="animate-pop absolute left-0 right-0 top-full z-50 mt-2 max-h-[62vh] overflow-auto rounded-lg bg-[#111] p-1.5 shadow-xl ring-1 ring-white/10 ring-inset">
          {visible.length === 0 ? (
            ...
            visible.map((h) => (
              <Link
                ...
                className="flex items-center gap-3 rounded-md px-3 py-2.5 transition-colors hover:bg-white/[0.06]"
              >
                <span
