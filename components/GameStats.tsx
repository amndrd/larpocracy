    ...
    <div className={clsx('flex flex-wrap items-center gap-2', className)}>
      ...
      <span
        className={clsx(
          ...
          b.serie > 0 ? 'bg-gold-2 text-gold' : 'bg-white/[0.06] text-ink-3',
        )}
      >
        <IconFlamme className="size-3.5" />
        <span className="tabular-nums">{b.serie}</span> j
      </span>
      <span className="inline-flex items-center gap-1.5 rounded-full bg-white/[0.06] px-3 py-1.5 text-[0.8125rem] font-semibold text-ink-2">
        ...
        >
          <span
            className={clsx(
              ...
              d.obtenue ? 'bg-gold-2 text-gold' : 'bg-white/[0.06] text-ink-3',
            )}
          >
